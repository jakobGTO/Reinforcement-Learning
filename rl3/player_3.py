...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        #Randomly generates margin unif[0,1]
        margin = np.random.uniform(0,1,1)
        #if the constant epsilon is less than margin, we choose max reward action from Q-table
        if margin >= epsilon: 
            action = np.nanargmax(Q[state])
        #else we choose a random action
        else:
            action = np.random.choice(all_actions)

    elif eps_type == 'linear':

        #Implementing linear epsilon value that has a linear decrease over time
        e_t = ScheduleLinear(anneal_timesteps,epsilon_final).value(current_total_steps)
        #Randomly generates margin unif[0,1]
        margin = np.random.uniform(0,1,1)
        #if the anealed epsilon is less than margin, we choose max reward action from Q-table
        if margin >= e_t:
            action = np.nanargmax(Q[state])
        #else we choose random action
        else:
            action = np.random.choice(all_actions)
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        # Initializing a numpy array with ns state rows and na state columns with float values from 0.0 to 1.0.
        Q = np.random.uniform(0,1,size=(ns,na))
        ...
        # incorporated a threshold limit, to stop training when the mean difference
        ...
        while diff >= self.threshold:
            ...
            while not end_episode:
                ...
                # Chose an action according to the epsilon greedy algorithm               
                ...
                # compute reward
                action_str = self.action_list[int(action)]
                ...
                # Implementing the Bellman Update equation to update Q
...
class ScheduleLinear(object):
    ...
    def value(self, t):
        # Return the annealed linear value for epsilon
        eps_delta = self.final_p - self.initial_p
        e_t = self.initial_p + eps_delta * (t/self.schedule_timesteps)
        return e_t
