...
class PlayerControllerRandom(PlayerController):
    ...
    def random_agent(self):
        ...
        current_total_steps = 0
        end_episode = False
        # Initializiing a numpy array with ns state rows and na state columns with zeros
        state_action_arr = np.zeros((ns,na))
        ...
        while episode <= self.episode_max:
            ...
            while not end_episode:
                ...
                # Randomly chooses an action from all possible actions and add to the counter of actions per state
                action = random.sample(possible_actions,1)[0]
                state_action_arr[s_current][action] += 1
            ...
            episode += 1
            end_episode = False
            #After 15 epsiodes, finished training
            if episode > 15:
                end_episode = True
        #Return the policy
        return self.get_policy(state_action_arr)
